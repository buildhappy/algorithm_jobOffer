...
public class TreeDistance {
	 ...
	 * @param root
	 */
	public static void maxDistanceRecursive(TreeNode root) {
		if (root == null) {
		    return;
        }
		if (root.left != null) {
			maxDistanceRecursive(root.left);
			root.maxLeft = Math.max(root.left.maxLeft, root.left.maxRight) + 1;
		}
		if (root.right != null) {
			maxDistanceRecursive(root.right);
	...
	public static void main(String[] args) {
		...
		maxDistanceRecursive(root);
		System.out.println("递归实现的结果：" + maxDistance);
		maxDistance = 0;
		maxDistance(root);
		System.out.println("非递归实现的结果：" + maxDistance);
	}
