 ...
 *  统计一个数字在排序数组中出现的次数。
 * 思路：
 *  由于是排序数组，要查找其中的一个数字，最简便的方法便是折半查找，
 *  这样我们可以先查找最先出现的位置和最后出现的位置，数出中间的个数便为总共的出现次数。
 * @author dell
 * 
 */
public class _038_GetNumOfK {
	...
	public static int getNumOfK(int array[], int k) {
		...
		if (array != null) {
            // 初始位置
			int index_start = getFirstK(array, k, 0, array.length - 1);
            // 结束位置
			int index_end = getLastK(array, k, 0, array.length - 1);
	...
	public static int getLastK(int array[], int k, int low, int high) {
        ...
        // 查找的值是为k，剩下的是判断是否是最后一个k
		if (array[middle] == k) {
			//是最后一个
			if ((middle > 0 && array[middle + 1] != k) || middle == array.length - 1) {
				return middle;
			} else {
                //不是最后一个
				low = middle + 1;
