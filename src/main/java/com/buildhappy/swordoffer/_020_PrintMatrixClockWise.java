 ...
 * @author buildhappy
 */
public class _020_PrintMatrixClockWise {
    //顺时针打印矩阵
    public static void printMatrixClockWisely(int[][] matrix, int rows, int colums) {
        ...
        for (int start = 0; ((start << 1) < rows) && ((start << 1) < colums); ++start) {
            printMatixInCircle(matrix, rows, colums, start);
        }
    }

    //打印矩阵的一圈,以(start,start)为起始点
    public static void printMatixInCircle(int[][] matrix, int rows, int colums, int start) {
