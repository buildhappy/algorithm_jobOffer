package com.buildhappy.swordOffer._027_BSTToList;
...
import com.buildhappy.swordOffer._025_PathInTree.PathOfTree.Tree;
/*
 * 把二元查找树转变成排序的双向链表
 * 题目：
...
public class BSTreeToLinkedList {
	public static void main(String[] args) {
		...
		int[] data = {10, 6, 14, 4, 8, 12, 16 };//这些数据是按二叉查找树的层次遍历存放
		Node head = bn.creatTree(data);
		//Node head = bn.createTreeIterator(data, 0);
		System.out.print("树的层序序列：" );
		levelPrintTree(head);
		Node node = bn.toTwoWayLinkedList(head);
	...
	public Node createTreeIterator(int[] data , int i){
	...
	private static void levelPrintTree(Node root) {
		if (root == null)
			return;
